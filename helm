...
helm search hub | wc -l #aranan sonuçta kaç satır var onu gösterir
```
...
```bash
helm install --debug --dry-run mydryrun techpro-chart  #dry-run ile yükleme yapmadan bize göster

k create deploy my -- image=nginx -- dry-run=client -o yaml > mydeploy.yaml
```
...
```bash
helm install --debug --dry-run setflag techpro-chart --set course=AWS #debug olmasada olur.Debug kodun doğruluğu için.
```
...
data:
  myvalue: "techpro-chart configmap example"
  course: {{ quote .Values.course }} #quote tırnak içine alıyor
  topic: {{ upper .Values.lesson.topic }}
  time: {{ now | date "2006.01.02" | quote }}  #sadece now da diyebilirsin
```
...
```bash
helm install notes-demo techpro- #realese oluşturur
```
...
chartmuseum --debug --port=8080 \
  --storage="local" \
  --storage-local-rootdir="./chartstorage"  #masterın depolama alanını kullanıyor
```
...
```bash
helm package techpro-chart  #ziplemek için 
```
...
helm repo index my-charts --url http://<public-ip>:8080 index yaml şeklinde bilgi sağlar
```
...
entries:
  ...
  - apiVersion: v2
    ...
    type: application
    urls:
    - http://<public ip>:8080/techpro-chart-0.1.0.tgz
    version: 0.1.0
...
```bash
cd my-charts
curl --data-binary "@techpro-chart-0.1.0.tgz" http://<public ip>:8080/api/charts #chartmusem repo ekler yani pushlar
```
...
```bash
helm package techpro-chart  #repoya göndermek için ziplemek zorundasın
mv techpro-chart-0.1.1.tgz my-charts #chart.yaml gittik versiyonu 0.1.1 yaptık ve zipleyip taşıdık
helm repo index my-charts --url http://<public-ip>:8080 #revizyon yaptık ve tekrar index.hml oluşturduk
```
...
```
helm plugin ls #yukardaki komutları kolaylaştırmak için
...
```bash
cd
helm cm-push techpro-chart mylocalrepo #yukardaki komutları tek satırda çalıştırır plugin sayesinde
```
...
```bash
helm upgrade from-local-repo mylocalrepo/techpro-chart #yeni relase yapma olanı güncelle
```
...
```bash
helm history from-local-repo
helm rollback from-local-repo 1 #önceki versiyona gitmek için
```
