...
$ kubectl apply -f hpa-web.yaml  #uyarı verir çünkü metriki göremiyor onun için bir kod yükleyeceğiz.
```
...
while true; do wget -q -O- http://<puplic ip>:<port number of php-apache-service>; done  #-q sessiz  modda çalış
                           #-O çıktıyı ekranda yazdır.
```
...
/ # while true; do wget -q -O- http://<puplic ip>:<port number of web-service> > /dev/null; done  #çıktıyı /dev/null ile çöp kutusuna yönlendirir.
#while true; do wget -q -O- http://3.231.219.18:30001 > /dev/null; done   dev/null dediğimiz çöp kutusuna yollar
#while true; do wget -q -O- http://3.231.219.18:30001 >> output; done   #çıktığı output adlı dosya oluşturup ona da gönderebiliriz
```
